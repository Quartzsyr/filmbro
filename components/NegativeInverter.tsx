...
export const NegativeInverter: React.FC<NegativeInverterProps> = ({ onClose }) => {
  ...
  const [orangeMask, setOrangeMask] = useState(30); // 橙色遮罩补偿
  ...
  const filterStyle = {
    filter: `invert(1) hue-rotate(${orangeMask}deg) brightness(${exposure}) contrast(${contrast})`,
  };

  return (
    <div className="fixed inset-0 z-[80] bg-black flex flex-col animate-fade-in font-display overflow-hidden">
      ...
      <main className="flex-1 relative flex items-center justify-center bg-[#050505]">
        ...
        <div className="absolute bottom-8 left-0 right-0 px-6 space-y-6 bg-gradient-to-t from-black via-black/80 to-transparent pt-12">
            <div className="space-y-4 max-w-sm mx-auto">
                <div className="flex items-center gap-4">
                    <span className="material-symbols-outlined text-orange-500 text-sm">palette</span>
                    <input 
                        type="range" min="0" max="360" value={orangeMask} 
                        onChange={e => setOrangeMask(parseInt(e.target.value))}
                        className="flex-1 accent-primary h-1 bg-white/10 rounded-full appearance-none"
                    />
                    <span className="text-[10px] font-mono text-muted w-8">遮罩</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="material-symbols-outlined text-blue-400 text-sm">light_mode</span>
                    <input 
                        type="range" min="0.5" max="2" step="0.1" value={exposure} 
                        onChange={e => setExposure(parseFloat(e.target.value))}
                        className="flex-1 accent-primary h-1 bg-white/10 rounded-full appearance-none"
                    />
                    <span className="text-[10px] font-mono text-muted w-8">曝光</span>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    <input 
                        ...
                        className="flex-1 accent-primary h-1 bg-white/10 rounded-full appearance-none"
                    />
                    <span className="text-[10px] font-mono text-muted w-8">反差</span>
                </div>
            </div>
            
            <p className="text-[9px] text-center text-muted uppercase tracking-widest pb-4">
                实时处理：反相 (Invert) + 橙色遮罩补偿 (Hue Correct)
            </p>
        </div>
      </main>
    </div>
